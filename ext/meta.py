...
class Meta(commands.Cog):
    ...
    async def prefix(self, ctx: commands.Context):
        ...
        prefix = ctx.bot.prefixes.get(ctx.guild.id, "'")
        set_usage = (
    ...
    async def invite(self, ctx: commands.Context):
        """Gives the invite link to add the bot to your server"""

        await ctx.send(
            embed=discord.Embed(
                title="Invite",
                description="https://discord.com/api/oauth2/authorize?client_id=718577828685152407&permissions=268435456&scope=bot",
            )
        )
